...
function Generator({
  ...
  const [referenceSettings, setReferenceSettings] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("image-generator-reference-settings") || "{}");
    } catch (_) {
      return {};
    }
  });
  const normalizedReferenceSettings = {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("image-generator-reference-settings", JSON.stringify(normalizedReferenceSettings));
    } catch (err) {
      console.warn("Could not persist reference settings", err);
    }
  ...
  const getBestReferenceSource = useCallback(() => {
    const rolePriority = { Appearance: 10, Source: 9, Character: 8, Face: 8, Product: 4, Pose: 3, Composition: 2, Style: 1 };
    const candidates = activeReferences
      .filter((item) => item.src && item.enabled !== false)
      .slice()
      .sort((a, b) => {
        const pa = rolePriority[a.role] || 0;
        const pb = rolePriority[b.role] || 0;
        return pb - pa || Number(b.pinned === true) - Number(a.pinned === true) || Number(b.weight || 0) - Number(a.weight || 0);
      });
  ...
  const buildReferenceStatusText = useCallback(() => {
    ...
    const roles = activeReferences.reduce((acc, item) => {
          ...
          <div className="m3-card">
            ...
            <div className="m3-field-group">
              ...
              <ReferenceManager
                ...
                onUseAsBase={handleUseReferenceAsBase}
                disabled={isGenerating}
              />

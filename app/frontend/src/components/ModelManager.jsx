...
const FEATURED_OPEN_MODELS = [
  {
    id: "moonshotai/Kimi-K3",
    name: "Kimi K3",
    publisher: "Moonshot AI",
    format: "Open-weight repository",
    approxSize: "Very large; advanced hardware required",
    notes: "Official native multimodal Kimi model. This repository is not a single GGUF file, so LUKE AI opens the official model page instead of pretending it can perform a one-click local install.",
    pageUrl: "https://huggingface.co/moonshotai/Kimi-K3",
    actionType: "open-page",
    tags: ["text", "vision", "open", "multimodal"],
    fitLabel: "Advanced hardware",
...
const CLOUD_AI_MODELS = [
  {
    ...
    publisher: "Anthropic",
    format: "Cloud API",
    approxSize: "No local download",
    notes: "Claude is connected through the Anthropic API. Model weights are not distributed as a downloadable local model.",
    pageUrl: "https://docs.anthropic.com/en/docs/about-claude/models/overview",
    actionType: "connect-cloud",
    tags: ["text", "vision", "cloud"],
  },
];
...
function ModelManager({ 
  ...
  const [textLibraryMode, setTextLibraryMode] = useState("text");
  const [videoCapability, setVideoCapability] = useState({ state: "checking", installed: false });
  const [videoCompatibility, setVideoCompatibility] = useState({ models: [] });
  ...
  const downloadPollingRef = React.useRef(null);
  ...
  const refreshVideoCapability = useCallback(async () => {
    try {
      const [status, compatibility] = await Promise.all([
        getImageToVideoCapabilityStatus(),
        getImageToVideoCompatibility(),
      ]);
      setVideoCapability(status || { state: "not-installed", installed: false });
      setVideoCompatibility(compatibility || { models: [] });
    } catch (error) {
      setVideoCapability({ state: "error", installed: false, message: error.message || String(error) });
    }
  }, []);

  useEffect(() => {
    if (activeModelType === "text" && textLibraryMode === "video") {
      refreshVideoCapability();
      const timer = setInterval(refreshVideoCapability, 2500);
      return () => clearInterval(timer);
    }
    return undefined;
  }, [activeModelType, textLibraryMode, refreshVideoCapability]);

  const handleVideoCapabilityAction = async (repair = false) => {
    setVideoActionBusy(true);
    try {
      await installImageToVideoCapability(repair);
      await refreshVideoCapability();
    } catch (error) {
      await showAlert({ title: repair ? "Repair Failed" : "Installation Failed", message: error.message || String(error), danger: true });
    } finally {
      setVideoActionBusy(false);
    }
  };

  const activeDownloadJobs = Object.values(downloadJobs).filter((job) => job?.active);
  const getDownloadJob = useCallback((...ids) => {
    const wanted = ids.filter(Boolean).map(String);
    return Object.values(downloadJobs).find((job) => wanted.includes(String(job.filename)) || wanted.includes(String(job.id)));
  }, [downloadJobs]);

  const contextWindow = Math.max(0, Math.min(20000, Number(textSettings?.contextSize ?? 0)));
  const updateContextWindow = (nextValue) => {
    const value = Math.max(0, Math.min(20000, Number.parseInt(nextValue, 10) || 0));
    if (typeof setTextSettings === "function") {
      setTextSettings((prev) => ({ ...(prev || {}), contextSize: value }));
    }
  };
  const adjustContextWindow = (delta) => updateContextWindow(contextWindow + delta);
  ...
  if (activeModelType === "image") {
    ...
    if (textLibraryMode === "video") {
      visibleModelLibrary = [];
    } else if (textLibraryMode === "cloud") {
      visibleModelLibrary = [{ group: "Cloud AI", items: CLOUD_AI_MODELS }];
    } else if (textLibraryMode === "open") {
      visibleModelLibrary = [{ group: "Featured Open Models", items: FEATURED_OPEN_MODELS }];
    } else if (textLibraryMode === "vision") {
      const visionItems = (hasHuggingFaceResults ? displayedHuggingFaceModels : TEXT_MODEL_LIBRARY[0].items)
        .filter((model) => model.tags?.includes("vision") || model.projectorUrl || /vision|llava|vl/i.test(`${model.name || ""} ${model.notes || ""}`));
      visibleModelLibrary = [{ group: "Vision AI Models", items: visionItems }];
    } else if (textLibraryMode === "installed") {
      visibleModelLibrary = [];
    } else {
      visibleModelLibrary = [{
    ...
    const speechItems = normalizedLocalModels.filter((model) => model.url).map((model) => ({
      ...model,
      format: "Whisper.cpp",
      approxSize: model.size,
      notes: model.notes || `${model.language || "Speech"} transcription model. Stored in app/speech-models/.`,
      recommendedTiers: model.recommended ? ["Low", "Mid", "High"] : [],
    }));
    visibleModelLibrary = [
      {
        group: "Thai Speech Models / โมเดลถอดเสียงภาษาไทย",
        items: speechItems.filter((model) => model.language === "Thai" || model.languageCode === "th"),
      },
      {
  ...
  const startProgressPolling = (modelId = "") => {
    if (modelId) setDownloadingModelId(modelId);
    if (downloadPollingRef.current) return;

    downloadPollingRef.current = setInterval(async () => {
      try {
        const status = await getDownloadProgress();
        const jobs = Array.isArray(status.downloads) ? status.downloads : (status.filename ? [status] : []);
        const byKey = {};
        jobs.forEach((job) => {
          const key = job.id || job.filename;
          if (key) byKey[key] = job;
        });
        setDownloadJobs(byKey);

        const activeJobs = jobs.filter((job) => job.active);
        if (activeJobs.length > 0 || status.active) {
          const first = activeJobs[0] || status;
          setDownloadingModelId(first.filename || modelId || null);
          setDownloadProgress(status.progress === -1 ? 0 : (Number(status.progress) || Number(first.progress) || 0));
          setDownloadSpeed(status.speed || `${activeJobs.length} active`);
        } else {
          clearInterval(downloadPollingRef.current);
          downloadPollingRef.current = null;
          setDownloadingModelId(null);
          setDownloadProgress(status.progress === -1 ? 0 : (Number(status.progress) || 0));
          setDownloadSpeed(status.speed || "");
          if (status.error && !String(status.error).toLowerCase().includes("cancelled")) {
  ...
  const handleCancelDownload = async (filename = "") => {
    try {
      ...
      if (!filename || downloadingModelId === filename) setDownloadingModelId(null);
  ...
  const performLoadModel = async (modelId, forcedConstraints = null) => {
    ...
    if (!isTauriDesktop && !isLocalServerMode()) {
      ...
      const cmd = isOpenVinoModel
        ? `Run the platform launcher and load "${modelId}" from AI Library on a configured Intel NPU machine.`
  ...
  const isUrlDownloading = activeDownloadJobs.length > 0;

  return (
    <div className="workspace-area">
      <div className="workspace-title-section">
        <h2 className="workspace-title">AI Library</h2>
      ...
      {activeModelType === "speech" && (
        <div className="m3-card" style={{ borderLeft: "4px solid var(--md-sys-color-primary)", marginBottom: "16px" }}>
          <h4 style={{ fontWeight: 700, marginBottom: "6px" }}>Thai Speech Support / รองรับภาษาไทย</h4>
          <p style={{ fontSize: "0.86rem", color: "var(--md-sys-color-on-surface-variant)", marginBottom: "8px" }}>
            Use a Thai-labeled Whisper model, then choose <strong>Thai / ภาษาไทย</strong> in Speech Transcriber for better Thai transcription accuracy.
          </p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
            <span className="status-chip" style={{ cursor: "default" }}>Recommended: Whisper Small Thai</span>
            <span className="status-chip" style={{ cursor: "default" }}>Language Code: th</span>
            <span className="status-chip" style={{ cursor: "default" }}>Folder: app/speech-models/</span>
          </div>
        </div>
      )}
      ...
      {activeModelType === "text" && (
        <div className="m3-card" style={{ marginTop: "24px", marginBottom: "16px" }}>
          <div className="workspace-title-section" style={{ marginBottom: "14px" }}>
            <h3 className="m3-card-title">Text Model Context Window</h3>
            <span className="settings-value-badge">{contextWindow.toLocaleString()} / 20,000</span>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "auto 1fr auto", gap: "12px", alignItems: "center" }}>
            <button
              ...
              style={{ height: "38px", minWidth: "44px", padding: "0 14px" }}
              onClick={() => adjustContextWindow(-512)}
              disabled={contextWindow <= 0}
              title="Decrease context by 512"
            >
              −
            </button>
            <input
            ...
            <button
              ...
              style={{ height: "38px", minWidth: "44px", padding: "0 14px" }}
              onClick={() => adjustContextWindow(512)}
              disabled={contextWindow >= 20000}
              title="Increase context by 512"
            >
              +
            </button>
          </div>
          ...
          <div style={{ display: "flex", gap: "10px", alignItems: "center", marginTop: "12px", flexWrap: "wrap" }}>
            <label style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "0.85rem", color: "var(--md-sys-color-on-surface-variant)" }}>
              Context
              <input
                type="number"
                min="0"
                ...
                value={contextWindow}
                onChange={(event) => updateContextWindow(event.target.value)}
                style={{ width: "120px", height: "34px", borderRadius: "10px", border: "1px solid var(--md-sys-color-outline-variant)", background: "var(--md-sys-color-surface)", color: "var(--md-sys-color-on-surface)", padding: "0 10px" }}
              />
            </label>
            <button type="button" className="m3-btn m3-btn-text" onClick={() => updateContextWindow(0)}>Auto / 0</button>
            <button type="button" className="m3-btn m3-btn-text" onClick={() => updateContextWindow(4096)}>4,096</button>
            <button type="button" className="m3-btn m3-btn-text" onClick={() => updateContextWindow(8192)}>8,192</button>
            <button type="button" className="m3-btn m3-btn-text" onClick={() => updateContextWindow(12000)}>12,000</button>
            <button type="button" className="m3-btn m3-btn-text" onClick={() => updateContextWindow(20000)}>20,000</button>
          </div>

          <p className="settings-option-desc" style={{ marginTop: "10px" }}>
            This value is saved to Text Chat settings and sent when loading GGUF text models. 0 keeps automatic/default context; maximum is capped at 20,000 tokens.
          </p>
        </div>
      )}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                    ...
                    <span style={{ fontSize: "0.75rem", opacity: 0.8 }}>
                      {activeModelType === "image"
                        ...
                        : activeModelType === "text"
                        ? `llama.cpp GGUF Model • Context ${contextWindow.toLocaleString()} / 20,000`
                        : activeModelType === "speech"
      ...
      <div className="workspace-title-section" style={{ marginTop: "32px", marginBottom: "16px" }}>
        <h3 className="m3-card-title">
          ...
          {activeModelType === "text" ? "AI Library" : activeModelType === "speech" ? "Speech Model Library" : activeModelType === "tts" ? "TTS Model Library" : "Model Library"}
        </h3>
      </div>

      {activeModelType === "text" && (
        <div className="ai-library-section-tabs" role="tablist" aria-label="AI Library sections">
          {TEXT_LIBRARY_SECTIONS.map((section) => (
            <button
              key={section.id}
              type="button"
              className={`model-filter-chip ${textLibraryMode === section.id ? "selected" : ""}`}
              onClick={() => {
                setTextLibraryMode(section.id);
                if (section.id === "vision") setSelectedFilters(["vision"]);
                else if (section.id !== "text") setSelectedFilters([]);
              }}
              aria-selected={textLibraryMode === section.id}
            >
              {section.label}
            </button>
          ))}
      ...
      {activeModelType === "text" && textLibraryMode === "video" && (
        <section className="video-ai-manager" aria-label="Video AI Manager">
          <div className="video-ai-hero">
            <div>
              <span className="video-ai-eyebrow">VIDEO AI MANAGER</span>
              <h3>Image-to-Video</h3>
              <p>Install, repair, and check everything required to animate images. LUKE AI STUDIO manages Python, Torch, Diffusers, FFmpeg, and the video pipeline for you.</p>
            </div>
            <div className={`video-ai-status status-${videoCapability.state || "unknown"}`}>
              <strong>{videoCapability.state === "ready" ? "Ready" : videoCapability.state === "installing" ? "Installing" : videoCapability.state === "error" ? "Needs repair" : "Not installed"}</strong>
              <span>{videoCapability.message || (videoCapability.installed ? "All required components are available." : "Required components have not been installed yet.")}</span>
            </div>
          </div>

          <div className="video-ai-actions">
            {videoCapability.state === "ready" ? (
              <button className="m3-btn m3-btn-tonal" onClick={() => handleVideoCapabilityAction(true)} disabled={videoActionBusy}>
                <RefreshCw size={16} className={videoActionBusy ? "progress-spinner" : ""} />
                Repair / Verify
              </button>
            ) : (
              <button className="m3-btn m3-btn-filled" onClick={() => handleVideoCapabilityAction(videoCapability.state === "error")} disabled={videoActionBusy || videoCapability.state === "installing"}>
                <DownloadCloud size={16} className={(videoActionBusy || videoCapability.state === "installing") ? "progress-spinner" : ""} />
                {videoCapability.state === "installing" ? "Installing…" : videoCapability.state === "error" ? "Repair Image-to-Video" : "Install Image-to-Video"}
              </button>
            )}
            <button className="m3-btn m3-btn-text" onClick={refreshVideoCapability}>Check again</button>
          </div>

          <div className="video-model-grid">
            {(videoCompatibility.models || []).map((model) => (
              <article key={model.id} className="video-model-card">
                <div className="video-model-card-head">
                  <div>
                    <h4>{model.name}</h4>
                    <span>{model.backends?.join(" • ") || "Video pipeline"}</span>
                  </div>
                  <span className={`compatibility-pill ${model.status || "blocked"}`}>{model.label || model.status}</span>
                </div>
                <p>{model.reason}</p>
                <div className="video-model-specs">
                  <span>Minimum RAM {model.minRamGb} GB</span>
                  <span>Minimum VRAM {model.minVramGb} GB</span>
                </div>
              </article>
            ))}
          </div>
          ...
          <div className="video-ai-note">
            <HardDrive size={18} />
            <div><strong>Storage aware</strong><span>Large video models can be stored on an external SSD or USB drive. Runtime files remain small and local for faster startup.</span></div>
          </div>
        </section>
      )}

      {activeModelType === "text" && textLibraryMode !== "video" && textLibraryMode !== "cloud" && textLibraryMode !== "open" && textLibraryMode !== "installed" && (
          ...
          <div key={section.group} className="m3-card" style={{ margin: 0 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "12px" }}>
              {section.items.map((model) => {
                ...
                const downloadJob = getDownloadJob(model.filename, model.projectorFilename, `${model.filename}.zip`);
                const downloading = Boolean(downloadJob?.active);
                ...
                return (
                  <div key={`${model.id || section.group}:${model.filename}`} style={{ display: "flex", flexDirection: "column", gap: "10px", padding: "14px", background: "var(--md-sys-color-surface-variant)", border: "1px solid var(--md-sys-color-outline-variant)", borderRadius: "var(--md-shape-corner-medium)" }}>
                    ...
                    {downloading && (
                      <div className="model-progress-section" style={{ marginTop: "2px" }}>
                        <div className="model-progress-label">
                          ...
                          <span>{Number(downloadJob?.progress) > 0 ? `${downloadJob.progress}%` : "Preparing"}</span>
                        </div>
                        <div className="model-progress-bar">
                          <div className="model-progress-fill" style={{ width: `${Math.min(100, Math.max(0, Number(downloadJob?.progress) || 0))}%`, transition: "width 0.2s ease" }}></div>
                        </div>
                        {downloadJob?.speed && (
                          <div style={{ fontSize: "0.7rem", color: "var(--md-sys-color-outline)", marginTop: "4px" }}>
                            {downloadJob.speed}{downloadJob.eta > 0 ? ` • ETA ${downloadJob.eta}s` : ""}
                          </div>
                        )}
                      </div>
                    )}
                      ...
                      onClick={() => {
                        ...
                        if (model.actionType === "connect-cloud") {
                          showAlert({
                            ...
                            message: "Claude uses the Anthropic API and cannot be downloaded as a local model. Add your Anthropic API key in Cloud AI settings, then test the connection.",
                          });
                          return;
                        }
                        handleLibraryDownload(model);
                      }}
                      disabled={(installed || downloading) && !model.actionType}
                    >
                      {model.actionType === "connect-cloud" ? <Library size={14} /> : model.actionType === "open-page" ? <Library size={14} /> : installed ? <HardDrive size={14} /> : downloading ? <RefreshCw className="progress-spinner" size={14} /> : <DownloadCloud size={14} />}
                    ...
                    >
                      {model.actionType === "connect-cloud" ? "Anthropic model documentation" : model.pageUrl ? "View source" : "Save to Downloads folder"}
                    </a>
                  </div>
                );
              })}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "16px", marginTop: "16px" }}>
        ...
        <div className="m3-card" style={{ display: "flex", flexDirection: "column", justifyContent: "center", margin: 0, padding: "20px" }}>
          ...
          {activeDownloadJobs.length > 0 && (
            <div className="model-progress-section" style={{ marginTop: "0px", marginBottom: "12px" }}>
              <div className="model-progress-label">
                <span>{activeDownloadJobs.length} active download{activeDownloadJobs.length > 1 ? "s" : ""}</span>
                <span>Parallel Mode</span>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginTop: "8px" }}>
                {activeDownloadJobs.map((job) => (
                  <div key={job.id || job.filename} style={{ padding: "8px", border: "1px solid var(--md-sys-color-outline-variant)", borderRadius: "var(--md-shape-corner-small)" }}>
                    <div className="model-progress-label">
                      <span style={{ fontSize: "0.75rem", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "220px" }} title={job.filename}>
                        {job.filename}
                      </span>
                      <span>{Number(job.progress) > 0 ? `${job.progress}%` : "Preparing"}</span>
                    </div>
                    <div className="model-progress-bar" style={{ margin: "4px 0" }}>
                      <div className="model-progress-fill" style={{ width: `${Math.min(100, Math.max(0, Number(job.progress) || 0))}%`, transition: "width 0.15s ease" }}></div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px" }}>
                      <span style={{ fontSize: "0.7rem", color: "var(--md-sys-color-outline)" }}>
                        {job.speed || "Downloading"}{job.eta > 0 ? ` • ETA ${job.eta}s` : ""}
                      </span>
                      <button type="button" className="m3-btn m3-btn-error" style={{ height: "28px", padding: "0 10px" }} onClick={() => handleCancelDownload(job.filename)}>
                        <Square size={12} />
                        <span>Stop</span>
                      </button>
                    </div>

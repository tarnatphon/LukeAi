...
function TextChat({ 
  ...
  const compactConversationContext = (allMessages, contextLimit, reservedTokens = 0) => {
    ...
    for (let index = allMessages.length - 1; index >= 0; index -= 1) {
      ...
      if (recentTokens + tokens <= usableBudget * 0.68 || recent.length < 6) {
    ...
    const summary = [
      ...
      ...unique(facts, 12).map((item) => `Persistent fact/preference: ${item}`),
      ...unique(decisions, 12).map((item) => `Prior decision/progress: ${item}`),
      ...unique(openItems, 8).map((item) => `Open item: ${item}`),
    ].join("\n");

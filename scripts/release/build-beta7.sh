...
VERSION="${LUKE_AI_BUILD_VERSION:-1.0.0-beta.7}"
...
IGNORED_BUILD_STATUS="$(
  printf '%s\n' "$WORKTREE_STATUS" |
  grep -vE     '^[?][?][[:space:]]+scripts/release/$|^[?][?][[:space:]]+scripts/release/build-beta7\\.sh$|^[ MARC?]{1,2}[[:space:]]+scripts/release/build-beta7\\.sh$' ||
  true
)"
...
if [ "$STATUS" -eq 0 ]; then
  SOURCE_VALIDATION_PACKAGING_MODE=0

  if [ -x "$PROJECT_DIR/app/tools/node-mac/bin/node" ] \
    && [ -x "$PROJECT_DIR/app/tools/node-mac/bin/npm" ] \
    && [ -x "$PROJECT_DIR/app/tools/node-mac/bin/npx" ] \
    && [ -f "$PROJECT_DIR/app/tools/node-mac/lib/node_modules/npm/package.json" ]; then
    SOURCE_VALIDATION_PACKAGING_MODE=1
  fi

  LUKE_AI_PACKAGING_MODE="$SOURCE_VALIDATION_PACKAGING_MODE" \

...
install_macos_whisper_from_homebrew() {
  ...
  # Homebrew formulae have used different executable names across versions.
  # Accept whisper-cli, whisper-cpp, or main, then normalize it to the
  # application path expected by the local server: app/speech-backend/mac/cpu/whisper-cli.
  local whisper_bin=""
  for candidate in "$whisper_prefix/bin/whisper-cli" "$whisper_prefix/bin/whisper-cpp" "$whisper_prefix/bin/main"; do
    if [[ -x "$candidate" ]]; then
  ...
  if [[ -z "$whisper_bin" ]]; then
    echo "   XX   Homebrew whisper-cpp did not provide whisper-cli/whisper-cpp/main under $whisper_prefix/bin" >&2
    return 1
  fi

  mkdir -p "$cpu_dest" "$lib_dest"
  cp "$whisper_bin" "$cpu_dest/whisper-cli"
  if [[ -x "$whisper_prefix/bin/whisper-server" ]]; then
    cp "$whisper_prefix/bin/whisper-server" "$cpu_dest/whisper-server"
  elif [[ -x "$whisper_prefix/bin/server" ]]; then
    cp "$whisper_prefix/bin/server" "$cpu_dest/whisper-server"
  fi
...
if [[ "$PLATFORM" == "Linux" ]]; then
  ...
  if [[ ! -x "$APP_DIR/speech-backend/mac/cpu/whisper-cli" ]]; then
    if [[ -x "$APP_DIR/speech-backend/mac/whisper-cli" ]]; then
      cp "$APP_DIR/speech-backend/mac/whisper-cli" "$APP_DIR/speech-backend/mac/cpu/whisper-cli" 2>/dev/null || true
    elif [[ -x "$APP_DIR/speech-backend/mac/whisper-cpp" ]]; then
      cp "$APP_DIR/speech-backend/mac/whisper-cpp" "$APP_DIR/speech-backend/mac/cpu/whisper-cli" 2>/dev/null || true
    elif [[ -x "$APP_DIR/speech-backend/mac/main" ]]; then
      cp "$APP_DIR/speech-backend/mac/main" "$APP_DIR/speech-backend/mac/cpu/whisper-cli" 2>/dev/null || true
    fi
    cp "$APP_DIR/speech-backend/mac"/whisper-server "$APP_DIR/speech-backend/mac/cpu/" 2>/dev/null || true
    cp "$APP_DIR/speech-backend/mac"/server "$APP_DIR/speech-backend/mac/cpu/whisper-server" 2>/dev/null || true

...
function Print-Header {
    ...
    Write-Host "   LUKE AI STUDIO      -  First-Time Setup" -ForegroundColor Cyan
...
if (-not $nodeReady) {
    ...
    if (-not $nodeReady) {
        Print-Fail "Portable Node.js install is incomplete or corrupted. Close any running LUKE AI STUDIO windows, delete app/tools/node-win, then run setup again."
        Read-Host; exit 1
    }
...
if (-not (Test-Path $npmCmd)) {
    ...
    Print-Fail "Close any running LUKE AI STUDIO windows, delete app/tools/node-win, then run setup again."
    Read-Host; exit 1
}

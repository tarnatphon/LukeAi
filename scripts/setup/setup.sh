#!/usr/bin/env bash
#
# LUKE AI STUDIO - Linux/macOS Setup Script
...
print_header() {
  echo ""
  echo "  ============================================================"
  echo "   LUKE AI STUDIO      -  $PLATFORM_LABEL First-Time Setup"
...
node_runtime_healthy() {
  [[ -x "$NODE_BIN" ]] || return 1
  [[ -f "$NPM_CLI" ]] || return 1
  "$NODE_BIN" --version >/dev/null 2>&1 || return 1
...
EOF
  cat > "$NODE_DIR/bin/npx" <<'EOF'
#!/bin/sh
basedir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
exec "$basedir/node" "$basedir/../lib/node_modules/npm/bin/npx-cli.js" "$@"
EOF
  chmod +x "$NODE_DIR/bin/npm" "$NODE_DIR/bin/npx"
}
...
if node_runtime_healthy; then
  ...
  print_ok "Portable Node.js already ready: $VERSION (npm $NPM_VERSION)"
else
  if [[ -e "$NODE_DIR" ]]; then
    print_warn "Existing Portable Node.js/npm is incomplete. Reinstalling it automatically."
    rm -rf "$NODE_DIR"
  fi
  mkdir -p "$TOOLS_DIR"
  ...
  # Always replace npm/npx symlinks with portable wrappers. This also makes
  # archives safe when ZIP extraction does not preserve symbolic links.
  create_npm_wrappers

  if ! node_runtime_healthy; then
    print_fail "Portable Node.js/npm install is incomplete after extraction."
    print_info "Expected npm CLI: $NPM_CLI"
    rm -rf "$NODE_DIR"
    exit 1
  fi
  ...
  VERSION=$("$NODE_BIN" --version)
  NPM_VERSION=$("$NODE_BIN" "$NPM_CLI" --version)
  print_ok "Portable Node.js ready: $VERSION (npm $NPM_VERSION)"
fi
